...
class GradienteGeometrico {
  static double calcularPresenteInfinito({required double A, required double G, required double I, required bool positivo}) {
    return positivo ? A / I + G / (I * I) : A / I - G / (I * I);
  }

  static double calcularValorFuturo(
      {required double A, required double i, required int n, required double g, required bool positivo}) {
  ...
  static double calcularValorPresente(
    ...
    double segundaParte = (g / i) * (((pow(1 + i, n) - 1) / (i * pow(1 + i, n))) - (n / pow(1 + i, n)));
